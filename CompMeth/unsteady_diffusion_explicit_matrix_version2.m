...
% conditions
    ...
    unew(i)=uold(i)+d*(uold(i+1)-3*uold(i)+2*U0_use);
        
    for i=2:1:N-1
        unew(i)=uold(i)+d*(uold(i+1)-2*uold(i)+uold(i-1));
    end
    
    i=N;
    unew(i)=uold(i)+d*(2*Ub-3*uold(i)+uold(i-1));

    % Exchange old and new
    uold=unew;
    uHalf(timeCount)=unew(round(length(unew)/2));
%         if(time>100*dt)
