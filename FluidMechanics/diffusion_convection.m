...
Nsmall=round(rho*u1*L/(2*gamma))
step=5;
Nlarge=2000;

for N=Nsmall:step:Nlarge
...
for j=2:1:N-1       %(Loop on number of cells that are not on boundary)
    ...
    ae=gamma/dx-flux/2;        %Value of ae in local axes
    aw=gamma/dx+flux/2;        %Value of aw in local axes
...
error=max(abs(sol'-uexact));

if(error <= 0.005)
    str = [' threshhold is = ',num2str(N)];
    disp(str)
    break
end
